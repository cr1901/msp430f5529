#[doc = "Register `USBOEPIFG` reader"]
pub struct R(crate::R<USBOEPIFG_SPEC>);
impl core::ops::Deref for R {
    type Target = crate::R<USBOEPIFG_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::convert::From<crate::R<USBOEPIFG_SPEC>> for R {
    fn from(reader: crate::R<USBOEPIFG_SPEC>) -> Self {
        R(reader)
    }
}
#[doc = "Register `USBOEPIFG` writer"]
pub struct W(crate::W<USBOEPIFG_SPEC>);
impl core::ops::Deref for W {
    type Target = crate::W<USBOEPIFG_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::ops::DerefMut for W {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl core::convert::From<crate::W<USBOEPIFG_SPEC>> for W {
    fn from(writer: crate::W<USBOEPIFG_SPEC>) -> Self {
        W(writer)
    }
}
impl W {
    #[doc = "Writes raw bits to the register."]
    pub unsafe fn bits(&mut self, bits: u8) -> &mut Self {
        self.0.bits(bits);
        self
    }
}
#[doc = "USB Output endpoint interrupt flags\n\nThis register you can [`read`](crate::generic::Reg::read), [`write_with_zero`](crate::generic::Reg::write_with_zero), [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`modify`](crate::generic::Reg::modify). See [API](https://docs.rs/svd2rust/#read--modify--write-api).\n\nFor information about available fields see [usboepifg](index.html) module"]
pub struct USBOEPIFG_SPEC;
impl crate::RegisterSpec for USBOEPIFG_SPEC {
    type Ux = u8;
}
#[doc = "`read()` method returns [usboepifg::R](R) reader structure"]
impl crate::Readable for USBOEPIFG_SPEC {
    type Reader = R;
}
#[doc = "`write(|w| ..)` method takes [usboepifg::W](W) writer structure"]
impl crate::Writable for USBOEPIFG_SPEC {
    type Writer = W;
}
#[doc = "`reset()` method sets USBOEPIFG to value 0"]
impl crate::Resettable for USBOEPIFG_SPEC {
    #[inline(always)]
    fn reset_value() -> Self::Ux {
        0
    }
}
